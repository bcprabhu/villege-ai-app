...
# REPLACE THE NUMBER BELOW WITH YOUR ACTUAL NUMBER (e.g., 919880000000)
...
with tab1:
    user_q = st.text_input("Ask a question / ಪ್ರಶ್ನೆ ಕೇಳಿ:", key="input_q")
    if st.button("Get Answer", key="q_btn"):
        if user_q:
            with st.spinner("Thinking..."):
                ...
                st.download_button("📥 Download Report", response.text, file_name="farming_advice.txt")
    ...
    with col1:
        if st.button("Mandi Prices"):
            with st.spinner("Fetching..."):
                ...
                st.download_button("📥 Save Prices", response.text, file_name="mandi_prices.txt")
    with col2:
